...
// ajfg
//  gcc -c predict.c  > errors.txt 2>&1
// macOS
// gcc  predict.c  -o predict -lncurses > errors.txt 2>&1

// Predict passes
/*

predict -f name       unix_start    unix_stop    -o output_file
           objectid

predict -f ISS 977446390 977446400 -o datafile


predict -f ISS +10    (seconds)
predict -f ISS +10m
...
Predict reads at the beginning:
...
predict  parameters.json


JSON:
{
    "messageCode": "MsgGetPasses",
    "startTime": "yyyy-mm-ddThh:mi:ss",
    "stopTime": "yyyy-mm-ddThh:mi:ss",
    "outputFile": "string",
    "outputType" : "int",
...
double CurrentDaynum()
{
    ...
    if (tz.tz_minuteswest==0)
        offset=qth.utc_offset/24.0;
    else
        offset=0.0;

    local_clock=((((double)tv.tv_sec+0.000001*(double)tv.tv_usec)/86400.0)-3651.0);

    return (local_clock-offset);
}

char *MyDaynum2String(double daynum)
{
    ...
    sprintf(timestr, "%d/%.2d/%.2d %.2d:%.2d:%.2d",
        1900 + tmpTime->tm_year, tmpTime->tm_mon, tmpTime->tm_mday, 
...
char *MyDaynum2StringDuration(double daynum)
{
    /* This function takes the given epoch as a fractional number of
       days since 31Dec79 00:00:00 UTC and returns the corresponding
       date as a string of the form "Tue 12Oct99 17:22:37". */
...
double daynum;
{
    double t1, t2, t3, ew, en, e, n, si, co, ra, gh,
        ...
        C1=0.91746406, S1=0.397818675, moon_az, moon_el;
    ...
    t1=(se+0.0657098*(daynum-de)+gm*1.002737909350795)/24.0;
    gs=24.0*(t1-floor(t1));
...
void ShowOrbitData()
{
    ...
    double an_period, no_period, satepoch, c1, e2, sma;
    char days[5];
...
void KepEdit()
{
    ...
    do
    {
        ...
        if (x!=-1)
        {
            ...
            clear();
            attrset(COLOR_PAIR(3)|A_BOLD);
            mvprintw(5,1,"\t\t   *  Orbital Database Editing Utility  *\n");
            attrset(COLOR_PAIR(4)|A_BOLD);
...
void QthEdit()
{
    ...
    mvprintw(15,43,"%2.1f",qth.utc_offset);
    refresh();

    sprintf(temp,"%s",qth.callsign);

    mvprintw(19,20,"Enter the callsign of your ground station");
    ...
    mvprintw(19,10,"Enter your latitude in decimal degrees NORTH (south=negative)");
    ...
    mvprintw(19,10,"Enter your longitude in decimal degrees WEST (east=negative) ");

    if (KbEdit(44,14))
        sscanf(temp,"%lf",&qth.stnlong);

    mvprintw(19,10,"       Enter your altitude above sea level (in meters)       ");
    ...
    mvprintw(19,4,"Enter the number of hours your system clock is relative to UTC (EST=-5.0)");

    sprintf(temp,"%2.1f",qth.utc_offset);

    if (KbEdit(44,16))
        sscanf(temp,"%f",&qth.utc_offset);
...
int x;
{
    ...
    char	going_or_coming=0, command[80], comsat, alarm=0,
    ...
    do
    {
        ...
        mvprintw(18+bshift,3,"%+6.2f%c  ",eclipse_depth/deg2rad,248);
        ...
        else if (sat_ele<0.0 && geostationary==0 && decayed==0 && aoshappens==1 && daynum>nextaos)
        {
...
void MultiTrack()
{
    ...
    double aos[24], aos2[24], temptime, nextcalctime=0.0;
...
void Illumination()
{
    ...
    char string1[40], string2[80], datestring[25], count;

    oneminute=1.0/(24.0*60.0);

    daynum=floor(GetStartTime());
    PreCalc(indx);
    startday=daynum;
    count=0;
...
void ProgramInfo()
{
    ...
    if (antfd!=-1)
        printw("\t\t\tAutoTracking    : Sending data to %s\n",serial_port);
    else
...
char *string, *outputfile;
{
    ...
    for (z=0; z<24; z++)
    {
        if ((strcmp(sat[z].name,satname)==0) || (atol(satname)==sat[z].catnum))
        {
            ...
            now=(long)((3651.0+CurrentDaynum())*86400.0);
...
time_t  GetTime(char *inputTime)
{
    ...
    sscanf(inputTime, "%d-%d-%dT%d:%d:%d", &tmpYear, &tmpMonth, &tmpDay, &tmpHour, &tmpMinute, &tmpSecond);


    struct tm  tmpTime;
    
    tmpTime.tm_year = tmpYear - 1900;
    tmpTime.tm_mon  = tmpMonth - 1;
    tmpTime.tm_mday = tmpDay;
    tmpTime.tm_hour = tmpHour;
    tmpTime.tm_min  = tmpMinute;
    tmpTime.tm_sec  = tmpSecond;
    //tmpTime.tm_isdst = -1;   // No effect
    
    setenv("TZ", "UTC", 1);

    /* Convert daynum to Unix time (seconds since 01-Jan-70) */
    output = mktime(&tmpTime);

    return output;
}

int MyQuickPredict(json_message_t *jsonMessage)
{
    int x, y, z, step=1;
    long start, now, end, count;
    char satname[50], startstr[20], endstr[20];
    ...
    char losText[25];
    char tcaText[25];
    char durationText[25];

    // Open output file. Otherwise, use the screen
    if (jsonMessage->outputFile[0])
        fd=fopen(jsonMessage->outputFile,"w");
    else
        fd=stdout;
    ...
    // Extracted from  ReadDataFiles();
    
    // QTH
    // ==============================
    strcpy(qth.callsign, "None");
    qth.callsign[strlen(qth.callsign)-1]=0;
    
    qth.stnlat  = jsonMessage->gsLatitude;
    qth.stnlong = jsonMessage->gsLongitude;
    qth.stnalt  = jsonMessage->gsAltitude;
    // All time are in UTC
    qth.utc_offset = 0;
    ...
    // Satellite
    // ==============================
    if (KepCheck(jsonMessage->tleLine1, jsonMessage->tleLine2))
    {       
        /* Copy TLE data into the sat data structure */
        strncpy(sat[0].name,  jsonMessage->satelliteName,24);
        strncpy(sat[0].line1, jsonMessage->tleLine1, 69);
        strncpy(sat[0].line2, jsonMessage->tleLine2, 69);

        /* Update individual parameters */
        InternalUpdate(0);
    }

    start = GetTime(jsonMessage->startTime);
    end = GetTime(jsonMessage->stopTime);
    
    printf("Start time: %s %ul\n", jsonMessage->startTime, start);
    printf("Stop time: %s %ul\n",  jsonMessage->stopTime, end);
    ...
    /* Prevent a list greater than
       5 days from being produced */

    if ((end-start)>(86400*5))
    {
        start=now;
        end=now-86400;
    }
    ...
    {        
        ...
        bool endLoop = false;
        /* Expected end date */
        double  tmpEnd = ((end/86400.0)-3651.0);
        
        /* Trap geostationary orbits and passes that cannot occur. */
        if (AosHappens(indx) && Geostationary(indx)==0 && Decayed(indx,daynum)==0)
        {
            //fprintf(fd, "\nUnix UTC       Date       Time     El    Az   Phase  Lat   Long   Range  Orbit\n");
            fprintf(fd, "----------------------------------------------------------------------------------------\n");
            fprintf(fd, "AOS                  TCA                  LOS                  Duration  Max El  Orbit\n");
            fprintf(fd, "----------------------------------------------------------------------------------------\n\n");
...
//                    }
                ...
                //if (endLoop == false)
                //{
                    if (lastel!=0 && endLoop == false)
                    {
                        daynum=FindLOS();
                        losTime = daynum;
                        Calc();

                        //fprintf(fd,"%.0f  %s  %4d  %4d  %4d  %4d  %4d  %4d  %6ld  %6ld  %c\n",floor(86400.0*(3651.0+daynum)),Daynum2String(daynum),iel,maxElev,iaz,ma256,isplat,isplong,irk,rv,findsun);
                        
                        strcpy(&aosText[0], MyDaynum2String(aosTime));
                        strcpy(&tcaText[0], MyDaynum2String(tcaTime));
                        strcpy(&losText[0], MyDaynum2String(losTime));
                        strcpy(&durationText[0], MyDaynum2StringDuration(losTime - aosTime));
                        
                        fprintf(fd, "%s  %s  %s  %s  %4d  %d\n", aosText,
                                                                 tcaText,
                                                                 losText,
                                                                 durationText,
                                                                 maxElev,
                    ...
                    if (daynum >= tmpEnd)
                        endLoop = true;                    
                //}
            }  while (AosHappens(indx) && Decayed(indx,daynum)==0 && endLoop == false);
        }
...
char *ReadJSONFile(const char *inputFileName)
{
    char  *buffer = NULL;
...
//int ProcessJSONFile(char *inputData, bool *inExitFlag)
int ProcessJSONFile(char *inputData)
{
    int returnCode = -1;
    ...
    if (inputData != NULL)
    {
        const nx_json *json = nx_json_parse(inputData, 0);
        ...
        if (json) 
        {
            ...
            RESPONSE.   JSON:
            {
                "listPasses": [
                     {   "identifier": "int",
                         "AOS": "YYYY-MM-DDTHH:mm:SS",
                         "LOS": "YYYY-MM-DDTHH:mm:SS",
                         "duration":    "int",
                         "maxElevation": "float"
                    },
                    {   "identifier": "int",
            ...
            */
            ...
            nx_json *messageCode = nx_json_get(json, "messageCode");
            
            strncpy(jsonMessage.messageCode, messageCode->text_value, 80);
            
            // Message code: 1  ==>   Get passes
            if (messageCode->type != NX_JSON_NULL && messageCode->type == NX_JSON_STRING &&
                strcmp(messageCode->text_value, "MsgGetPasses") == 0)
            {
                strncpy(jsonMessage.outputFile, nx_json_get(json, "outputFile")->text_value, 80);
                //int    outputType = 0;   // 0 = text, 1 = JSON
                jsonMessage.outputType = nx_json_get(json, "outputType")->int_value;
                strncpy(jsonMessage.startTime, nx_json_get(json, "startTime")->text_value, 80);
                strncpy(jsonMessage.stopTime, nx_json_get(json, "stopTime")->text_value, 80);

                if (jsonMessage.outputFile[0] != 0 && 
                    jsonMessage.startTime[0] != 0 &&
                    jsonMessage.stopTime[0] != 0)
                {
                    strncpy(jsonMessage.satelliteName, nx_json_get(json, "satelliteName")->text_value, 80);
                    strncpy(jsonMessage.tleLine1, nx_json_get(json, "tleLine1")->text_value, 80);
                    strncpy(jsonMessage.tleLine2, nx_json_get(json, "tleLine2")->text_value, 80);
                    jsonMessage.gsLatitude = nx_json_get(json, "gsLatitude")->dbl_value;
                    jsonMessage.gsLongitude = nx_json_get(json, "gsLongitude")->dbl_value;
                    jsonMessage.gsAltitude = nx_json_get(json, "gsAltitude")->dbl_value;
           
                    // Calculate Passes
                    MyQuickPredict(&jsonMessage);                    
                }
                else
                {
                    // Key not found
...
char argc, *argv[];
{
    ...
    /* Set up default qth and tle filenames */

    strcpy(qthfile,"predict.qth");
    strcpy(tlefile,"predict.tle");
    strcpy(dbfile,"predict.db");

    /* Scan for optional command line arguments */

    // ajfg
    // If there is only 1 argument, it is a JSON file with the parameters
    if (argc == 2) {
        /*
        if (strcmp(argv[1], "zmq") == 0)
        {
            ...
            while (exitFlag == false) 
            {
                ...
                zmq_recv (responder, jsonText, 8192, 0);
                
                printf ("Received message\n");
                ProcessJSONFile(jsonText, &exitFlag);
                
                zmq_send (responder, "World", 5, 0);
            }
        } else {
            printf("ERROR: Incorrect input parameter. It should be 'zmq'\n");
        }
        */
        ...
        for (x=0; x<=128; jsonFileName[x]=0, x++);

        strncpy(jsonFileName, argv[1], 127);

        // Read JSON file
        jsonText = ReadJSONFile(jsonFileName);

        if (jsonText != NULL)
        {
            // Process file
            exit( ProcessJSONFile(jsonText) );
        }
    } else {

        for (x=0; x<=y; x++)  /* We need to start with x=0 for DOS */
        {
            // ajfg: Calculate passes
