...
public:
	enum {
        InvalidEndian, InvalidByteWidth, InvalidNumberBits
	};
...
public:
	std::string toString() {
		...
		switch(status)
		{
            ...
            case InvalidNumberBits:
                result = "InvalidNumberBits";
                break;
...
public:
	...
	uint32_t pop(uint32_t nBits, bool reverse = false) {
		if (nBits == 0 || nBits > 32) {
            throw BitstreamException(BitstreamException::InvalidNumberBits);
		}
		uint32_t ret = 0;
		if (popFromMSB) {
