...
ULONG Worksheet::CellTable::RowBlock::Read(const char* data)
{
    ...
    while(code != CODE::DBCELL) {
        switch(code) {
            ...
            case CODE::RK:
            case CODE::FORMULA:
                cellBlocks_.push_back(std::shared_ptr<CellBlock>(new CellBlock()));
...
BasicExcelWorksheet* BasicExcel::AddWorksheet(const char* name, int sheetIndex)
{
    ...
    if (sheetIndex == -1) {
        ...
        // ajfg
        yesheets_.push_back(std::shared_ptr<BasicExcelWorksheet>( new BasicExcelWorksheet(this, (int)worksheets_.size()-1)));
        ...
        // ajfg
        yesheet = &**(yesheets_.insert(yesheets_.begin()+sheetIndex, std::shared_ptr<BasicExcelWorksheet>(new BasicExcelWorksheet(this, sheetIndex))));
...
void BasicExcel::UpdateYExcelWorksheet()
{
    ...
    for(int i=0; i<maxWorksheets; ++i) {
        // ajfg
        yesheets_.push_back( std::shared_ptr<BasicExcelWorksheet>(new BasicExcelWorksheet(this, i)));
...
void BasicExcel::UpdateWorksheets()
{
    ...
    for(int s=0; s<maxWorksheets; ++s) {
        ...
        //ajfg
        vector<std::shared_ptr<Worksheet::CellTable::RowBlock::CellBlock> >* pCellBlocks;
...
//				if (cellType != BasicExcelCell::UNDEFINED)	// Current cell contains some data
                // Write cell content, even if blank in order to keep format
                {
                    ...
                    // ajfg
                    pCellBlocks->push_back( std::shared_ptr<Worksheet::CellTable::RowBlock::CellBlock>(new Worksheet::CellTable::RowBlock::CellBlock));
...
static void calculate_dimension(vector<Worksheet::CellTable::RowBlock>& rRowBlocks, int& maxRows_, int& maxCols_)
{
    ...
    for(size_t i=0; i<rRowBlocks.size(); ++i) {
        ...
        // ajfg
        vector<std::shared_ptr<Worksheet::CellTable::RowBlock::CellBlock> >& rCellBlocks = rRowBlocks[i].cellBlocks_;
...
void BasicExcelCell::SetFormula(const Worksheet::CellTable::RowBlock::CellBlock::Formula& f)
{
    ...
    _pFormula = shared_ptr<Formula>(new Formula(f));
}
