...
ServerMessage GetGsnMessage::processMessage(const ServerMessage &inMessage, ServerWorker &inParent)
{
    ...
    if (inMessage.getOperationCode() == OperationCodeEnum::CODE_GET_GROUND_STATION) {
        ...
        try {
            ...
            if (avroInputMessage.groundStationId.empty() == false) {
                groundStation = GsDatabase::getInstance().getById(avroInputMessage.groundStationId);
