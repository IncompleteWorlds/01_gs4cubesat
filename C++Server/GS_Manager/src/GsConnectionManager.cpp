...
GsConnectionManager::GsConnectionManager()
    : io_service{}, nextId{0}, activeConnections{}
{
    ...
    thread ioServiceThread(std::bind(&GsConnectionManager::runIoService, this));
...
GsConnection::PtrGsConnection GsConnectionManager::create(const string &inGsId)
{
    ...
    switch(gs.connectionType)
    {
        case IW::ConnectionTypeEnum::TCP:
            {
                // Create the object
                shared_ptr<GsTcpConnection> tmpConnection{ new GsTcpConnection(this->io_service, tmpId) };
