...
IW::TM_ProcessedFrame Ax25Layer::indication(RawFrameType& inRawData)
{
...
//    been completed.
IW::TM_ProcessedFrame Ax25Layer::confirm(RawFrameType& inRawData)
{
    IW::TM_ProcessedFrame  outputFrame;
...
int Ax25Layer::decodeFrame(RawFrameType& inData, IW::TM_ProcessedFrame &outData)
{
    int output = -1;
                ...
                if (processFCS(inData, fieldCounter, ax25Group) == 0)
                {
                    ...
                    if (processControlFlag(bs, fieldCounter, ax25Group) == 0)
                    {
                        ...
                        if (processPID(bs, fieldCounter, ax25Group) == 0)
                        {
                            ...
                            // Add group
                            outData.groups.get_array().push_back(ax25Group);
