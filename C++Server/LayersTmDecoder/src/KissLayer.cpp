...
IW::TM_ProcessedFrame KissLayer::indication(RawFrameType &inRawData)
{
    IW::TM_ProcessedFrame  kissFrame;
...
IW::TM_ProcessedFrame KissLayer::confirm(RawFrameType &inRawData)
{
    IW::TM_ProcessedFrame  kissFrame;
...
// Decode a KISS frame
int KissLayer::decodeFrame(RawFrameType& inData, IW::TM_ProcessedFrame& outData)
{
    int output = -1;
    ...
    if (inData.size() >  0)
    {
        ...
        // Check minimum size; FEND, frame type and FEND
        if (frameLength < 3)
        {
            LogManager::getInstance().error("Invalid KISS frame length. Minimum length is 3");
            output = -1;
        }
        else
        {
            ...
            {
                LogManager::getInstance().error("Invalid KISS frame. Start and end delimiters not found");
                output = -1;
            }
            else
            {
                ...
                if ((inData[1] & 0x0F) != 0)
                {
                    LogManager::getInstance().warning("Only data frames are implemented");
                }
                else
                {
                    // Create new group
                    ...
                    for (std::size_t i = 2; i < frameLength - 1; i++)
                    {
                        if (inData[i] == KISS_FESC)
                        {
                            ...
                            {
                                string  tmpByteString = toHexString<uint8_t>(inData[i+1]);
                                LogManager::getInstance().error("Invalid character in KISS frame " + tmpByteString);
                                output = -1;
                    ...
                    // Add group
                    outData.groups.get_array().push_back(kissGroup);
