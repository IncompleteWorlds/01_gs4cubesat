...
import com.incomplete.cubegs.mainwui.messages.M_CLoginMessage;
import com.incomplete.cubegs.mainwui.messages.M_CLoginResponse;
...
public class LoginPage extends BaseClassPage {
    ...
    private void doLogin() {
        ...
        if (textUserName.getText().isEmpty() == false && textPasswd.getText().isEmpty() == false) {
            ...
            try {
                M_CLoginMessage loginMsg = new M_CLoginMessage(textUserName.getText(), textPasswd.getText());
                
                // Send the message. Store the answer
                M_CLoginResponse loginResponse = HttpManager.sendRequest(loginMsg, M_CLoginResponse.class);
                                
                if (loginResponse.getStatus() == HttpURLConnection.HTTP_OK) {                
                    // Store current user
                    //ServletContext servletCtx = RWT.getUISession().getHttpSession().getServletContext();
                    UISession uiSession = RWT.getUISession();
                   
                    uiSession.setAttribute(Constants.USER_NAME,    textUserName.getText());
                    uiSession.setAttribute(Constants.USER_ID,      loginResponse.getUserId());
                    uiSession.setAttribute(Constants.ROLE_ID,      loginResponse.getRoleId());
                    //uiSession.setAttribute(Constants.MISSION_NAME, loginResponse.getMissionName());
                    uiSession.setAttribute(Constants.MISSION_NAME, "TODO");
                    uiSession.setAttribute(Constants.JWT_TOKEN,    loginResponse.getJwtToken());
                    
                    // Change view
                    SingletonViews.getInstance().changeView(Constants.MAIN_VIEW_NAME);
                    
                } else {
                    errorFlag = true;
                    errorMessage = loginResponse.getDetail();
                }
            } catch (Exception e) {
